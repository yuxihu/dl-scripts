...
docker run debian:buster /bin/sh -c "sleep 3600" &
sleep 5
...
docker exec ${CONTAINER} /bin/sh -c "apt-get install -y python3.6 python3.6-dev python3-distutils"
docker exec ${CONTAINER} /bin/sh -c "ln -sf /usr/bin/python3.6 /usr/bin/python"
